...
def upgrade() -> None:
    ...
    op.create_table(
        "course_node_content",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("node_id", sa.Integer(), sa.ForeignKey("course_nodes.id", ondelete="CASCADE"), nullable=False, unique=True, index=True),
    ...
    op.create_table(
        ...
        sa.Column("node_id", sa.Integer(), sa.ForeignKey("course_nodes.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("course_id", sa.Integer(), sa.ForeignKey("courses.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("node_id", sa.Integer(), sa.ForeignKey("course_nodes.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("quiz_session_id", UUID(as_uuid=False), sa.ForeignKey("quiz_sessions.id", ondelete="CASCADE"), nullable=False),
    )
