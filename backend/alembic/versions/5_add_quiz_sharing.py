...
def upgrade() -> None:
    op.add_column('quiz_sessions', sa.Column('share_code', sa.String(length=16), nullable=True))
    op.add_column('quiz_sessions', sa.Column('shared_to_plaza_at', sa.DateTime(), nullable=True))
    op.create_index('ix_quiz_sessions_share_code', 'quiz_sessions', ['share_code'], unique=True)
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('user_id', 'session_id', name='uq_quiz_acquisitions_user_session'),
    )
    op.create_index('ix_quiz_acquisitions_user_id', 'quiz_acquisitions', ['user_id'])
    op.create_index('ix_quiz_acquisitions_session_id', 'quiz_acquisitions', ['session_id'])
