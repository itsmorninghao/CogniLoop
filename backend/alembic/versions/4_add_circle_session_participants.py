...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "session_id", "user_id", name="uq_circle_session_participants_session_user"
        ),
    )
    op.create_index(
        "ix_circle_session_participants_session_id",
        "circle_session_participants",
        ["session_id"],
    )
    op.create_index(
        "ix_circle_session_participants_user_id",
        "circle_session_participants",
        ["user_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_circle_session_participants_user_id",
        table_name="circle_session_participants",
    )
    op.drop_index(
        "ix_circle_session_participants_session_id",
        table_name="circle_session_participants",
